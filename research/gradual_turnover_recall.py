...
event_hits={w:{name:[] for name in rules} for w in windows}
miss_feature_rows=[]
...
for ev in targets.itertuples(index=False):
    ...
    whole=grp[(grp['stock_idx']>=p-20)&(grp['stock_idx']<=p+5)]
    pre=whole[whole['stock_idx']<=p]
    if pre.empty:
        continue
...
for wname in windows:
    ...
    for name in rules:
        ...
        rows.append({
            ...
            'rule_hits':int(h.sum()),'rule_recall':h.mean(),
            'r1_hits':int(r1.sum()),'r1_recall':r1.mean(),
            'incremental_hits_over_r1':int((~r1 & h).sum()),
            'incremental_recall_over_r1':float((~r1 & h).sum()/n),
            'union_hits':int(union.sum()),'union_recall':union.mean(),
